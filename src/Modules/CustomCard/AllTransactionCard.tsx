...
import { getMandateAuthThunk } from '../../Store/Payments/thunk/payments-thunk';
...
function AllTrancationCard(props: transactionList) {
  ...
  const [initializePaymentGateway, setInitializePaymentGateway] = useState<boolean>(false);

  const handleMandateAuth = async (orderId: string) => {


    console.log("handleMandateAuth)()")

    // navigate("/netbanking");
    let res: apiResponse = await getMandateAuthThunk(`${siteConfig.PAYMENT_MANDATE_AUTH}?order_id=${orderId}`);

    console.log(res);

    navigate("/initializepaymentgateway", { state: { urlRedirect: "https://www.bsestarmf.in/ENACHResponse.aspx", appUrl: "/transactions" } })


    setInitializePaymentGateway(true);
  }
  ...
  return (
    <>
      ...
      <Grid>
        ...
        <Modal open={openMandateModal} onClose={() => setOpenMandateModal(false)}>
          <Box style={{
            ...
            <Button variant="contained" style={style.button3} fullWidth onClick={() => {
              handleMandateAuth(props?.order_id);
              setOpenMandateModal(false);
        ...
        <Modal open={openPaymentModal} onClose={() => setOpenPaymentModal(false)}>
          <Box style={{
            ...
            <Grid container xs={12} style={{ backgroundColor: "white", width: "100%" }}>
              ...
              <Grid item xs={6}><Typography style={{ fontSize: "14px", fontWeight: "500", color: '#3c3e42', padding: "10px 20px" }}>{objBankDetails?.accounttype}</Typography></Grid>
            </Grid>
            <Grid container xs={12} style={{ backgroundColor: "rgba(0, 0, 0, 0.05)", width: "100%" }}>
      ...
      {/* {
        initializePaymentGateway === true ?
          <InitializePaymentGateway
            urlRedirect="https://www.bsestarmf.in/ENACHResponse.aspx"
            urlApp={"/transactions"}
          />
          : null
      } */}
