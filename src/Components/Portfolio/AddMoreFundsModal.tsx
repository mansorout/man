...
const AddMoreFundsModal = (props: any) => {
  ...
  const [openMonthlyDatePicker, setOpenMonthlyDatePicker] = React.useState(false);
        ...
        <DialogContent>
          {!showPlanDetailSubmit ? (
            <div className={classes.showPlanDialogWrapper}>
              ...
              {
                        openMonthlyDatePicker ? <>
                        <Box sx={{ paddingTop: "0px" }}>
                        <ul className={classes.quickSelectWrapper}>
                          {quickPickAmount.map((item, index) => (
                            <li key={index} onClick={selectFromQuickPick} style={{marginTop:"-7%"}}>
                              {item}
                            </li>
                          ))}
                        </ul>
