...
import {
  setMasterFundListForExploreFundsAction,
  setSelectedFundsForInvestmentAction,
  setReplaceFundsForExploreFundsAction,
  setSelectedFundsForExploreFundsAction,
} from "../../Store/Recommendations/actions/recommendations-action";
import {
  setOrderSipThunk,
  setPlaceLumpsumOrderThunk,
} from "../../Store/Payments/thunk/payments-thunk";
...
const SelectedFunds = () => {
  ...
  const g_selectedFunds = useSelector(
    (state: any) =>
      state?.recommendationsReducer?.masterFundListForExploreFunds.data
  ); // select funds
  const g_selectedFundsForExploreFunds = useSelector(
    (state: any) => state?.recommendationsReducer?.selectedFundsForExploreFunds
  ); // add & replace
  const g_replaceForExploreFunds = useSelector(
    (state: any) =>
      state?.recommendationsReducer?.replaceFundActiveIndexForExploreFund?.data
  );
  ...
  const handleNavigation = (selectedFundAction: string) => {
    navigate("/explorefunds", {
      state: { status: selectedFundAction, parentRoute: "/home" },
    });
  };
  ...
  useEffect(() => {
    if (!g_selectedFunds?.length)
      handleNavigation(globalConstant.CEF_EXPLORE_FUND);
    ...
    for (var key in g_selectedFunds) {
      ...
      const validate =
        selected === 1
          ? g_selectedFunds[key].userRecommendedAmount >=
            g_selectedFunds[key].lumpsumminamount
            ? true
            : false
          : g_selectedFunds[key].userRecommendedAmount >=
            g_selectedFunds[key].sipminamount
          ? true
          : false;

      if (
        g_selectedFunds[key].hasOwnProperty(
          defaultKeys.userRecommendedAmount
        ) &&
        g_selectedFunds[key].userRecommendedAmount &&
        validate
      ) {
  ...
  useEffect(() => {
    console.log(
      ...
      "explore fund screen and useEffect of g_selectedFundsForExploreFunds"
    );

    // debugger
    if (
      g_selectedFundsForExploreFunds &&
      g_selectedFundsForExploreFunds?.data &&
      g_selectedFundsForExploreFunds?.data?.length
    ) {
      if (
        g_selectedFundsForExploreFunds &&
        g_selectedFundsForExploreFunds?.data &&
        g_selectedFundsForExploreFunds?.data[0]?.isChecked === false &&
        g_selectedFundsForExploreFunds?.data[0]?.fundSelected &&
        (g_selectedFundsForExploreFunds?.data.length === 1 ||
          g_selectedFundsForExploreFunds?.data.length > 1)
      ) {
        // debugger
        // For Add Fund
        ...
        addMoreFundTemp &&
          addMoreFundTemp?.length &&
          dispatch(setMasterFundListForExploreFundsAction(addMoreFundTemp));
        ...
        if (
          g_selectedFunds &&
          g_selectedFunds?.length &&
          g_replaceForExploreFunds
        ) {
  ...
  const handleRemoveBtn = (selectedFundAction: any) => {
    const temp =
      selectedFundsList &&
      selectedFundsList?.length &&
      selectedFundsList.filter(
        (item: any) => item?.secid !== selectedFundAction?.secid
      );
                    ...
                    <Breadcrumbs aria-label="breadcrumb">
                      <Link
                        ...
                        onClick={() =>
                          navigate("/explorefunds", {
                            state: {
                              status: globalConstant.CEF_EXPLORE_FUND,
                              parentRoute: "/home",
                            },
                          })
                        }
                          ...
                          selectedFundsList?.map((selectedFund: any) => (
                            <FundAmtCard
                              ...
                              replaceBtnAction={(item) => {
                                handleNavigation(
                                  globalConstant.CEF_REPLACE_OF_EXPLORE_FUND
                                );
                                dispatch(
                                  setReplaceFundsForExploreFundsAction(item)
                                );
                              }}
                      ...
                      <Grid container>
                        <Grid xs={12} md={12} textAlign="center">
                          <Button
                            onClick={() =>
                              handleNavigation(
                                globalConstant.CEF_ADD_FUND_OF_EXPLORE_FUND
                              )
                            }
                            sx={{
                    ...
                    <Grid item xs={12} md={6} sx={{ textAlign: "center" }}>
                      ...
                      >
                        ...
                        >
                          ...
                          <Box
                            onClick={() => {
                              setOnetimeLumpsum(false);
                              setSelected(0);
                              setFundList(
                                ExploreFundsList.filter(
                                  (item) => item.type == "Equity"
                                )
                              );
                            }}
                            style={{
